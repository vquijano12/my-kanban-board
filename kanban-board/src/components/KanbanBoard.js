  ...
  const moveTask = (taskId, sourceColumn, targetColumn) => {
    setTasks(prevTasks => {
      ...
      if (!prevTasks[sourceColumn] || !prevTasks[targetColumn]) {
        console.error(`One of the columns ${sourceColumn} or ${targetColumn} does not exist`);
        return prevTasks;
      }
  
      // Find the task to move
      const taskToMove = prevTasks[sourceColumn].find(task => task.id === taskId);
      if (!taskToMove) {
      ...
      // Perform the move
      const sourceTasks = prevTasks[sourceColumn].filter(task => task.id !== taskId);
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="KanbanBoard">
        <Column title="To Do" tasks={tasks.todo} addTask={addTask} deleteTask={deleteTask} moveTask={moveTask} />
        <Column title="In Progress" tasks={tasks.inProgress} deleteTask={deleteTask} moveTask={moveTask} />
        <Column title="Done" tasks={tasks.done} deleteTask={deleteTask} moveTask={moveTask} />
      </div>
    </DndProvider>
  );
}
